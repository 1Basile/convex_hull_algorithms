...
void  Voronoi::insert_parabola(VPoint * p)
{
  ...
  if(root->isLeaf && root->site->y - p->y < 1)  // Both on the same line
  {
    ...
    VPoint* s = new VPoint((p->x + fp->x)/2, height); // Make a point between points
    points.push_back(s);
  ...
  VPoint* start = new VPoint(p->x, get_y_of_point(par->site, p->x));
...
void  Voronoi::remove_parabola(VEvent * e)
{
  ...
  VPoint* p = new VPoint(e->point->x, get_y_of_point(p1->site, e->point->x)); // Add circle center as vertice to points
  points.push_back(p);
...
void  Voronoi::finish_edge(VParabola* n)
{
  ...
  VPoint* end = new VPoint(mx, mx * n->edge->a + n->edge->b); 
  n->edge->end = end;
...
double  Voronoi::get_y_of_point(VPoint * p, double x) // Get y coordiante of this point parabola for this x
{
...
VPoint* Voronoi::get_edge_intersection(VEdge* a, VEdge* b)
{
